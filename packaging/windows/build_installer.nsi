...
!define MUI_FINISHPAGE_RUN	$INSTDIR\bin\rascal.exe
...
Section "RasCAL 2" sec0
	...
	AccessControl::GrantOnFile "$INSTDIR\bin\_internal\matlab\engine\_arch.txt" "(BU)" "FullAccess"
	...
	CreateShortCut "$SMPROGRAMS\${PRODUCT}\${PRODUCT}.lnk" "$INSTDIR\bin\rascal.exe" "" "$INSTDIR\bin\rascal.exe" 0
...
!insertmacro MUI_FUNCTION_DESCRIPTION_BEGIN
	!insertmacro MUI_DESCRIPTION_TEXT ${sec0} "Install main rascal component."
...
Function create_desktop_shortcut
	CreateShortcut "$DESKTOP\${PRODUCT}.lnk" "$INSTDIR\bin\rascal.exe"
FunctionEnd
